...
public class DoorbellActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mCamera.takePicture();
       // log = mDatabase.getReference("logs").push();
            ...
            public void run() {
              ...
              //  byte[] imageBytes = new byte[1024];
               ...
               try {
                    ...
                    Log.d(TAG, "cloud vision annotations:" + imageBytes);
                    if (imageBytes != null) {
                        log.child("annotations").setValue(imageBytes);
                    }
                } catch (IOException e) {
    ...
    private void onPictureTaken(final byte[] imageBytes) {
        if (imageBytes != null) {
            final DatabaseReference log = mDatabase.getReference("image").push();
            String imageStr = Base64.encodeToString(imageBytes, Base64.NO_WRAP | Base64.URL_SAFE);
            // upload image to firebase
            log.child("timestamp").setValue(ServerValue.TIMESTAMP);
            log.child("images").setValue(imageStr);
    ...
    public  byte[] convertImageToByteArray(){
        Drawable d = ContextCompat.getDrawable(this, R.drawable.login_logo_h); // the drawable (Captain Obvious, to the rescue!!!)
